...
**第三轮（2026-08-17）**：socks5h 代理 scheme 已确认在后端允许列表（补齐两处提示文案含设置占位符）；i18n 第一批落地——设置"保存与性能"分区 13 键 + 任务级代理编辑器复用 netProxy* 键组并新增 netProxyModeGlobal（网络分区核实已全量国际化）。存量硬编码降至约 431 行（JSX 203 / 属性 148 / toast 80），后续批次按分区推进。
...
## 🧲 BT/磁力下载（2026-08-16 纳入，2026-08-18 正式升级为纯 Rust 原生内核）

架构决策：升级为纯 Rust 原生 `librqbit` 引擎，由后端直接进程内托管，零外部进程依赖，开箱即用。做种默认关闭；磁力元数据获取前不伪造文件名/大小；实时瞬时速度采样；支持文件选择下载（`only_files`）；多路公共 HTTP/HTTPS/UDP Tracker 自动注入。

| 任务 | 描述 | 状态 | 领域 |
| :--- | :--- | :--- | :--- |
| BT-01 | 纯 Rust 内核：librqbit 9.0 进程内托管、会话单例管理、零外部依赖开箱即用 | ✅ 完成（含单测） | backend |
| BT-02 | 磁力增强：全球与国内高可用 HTTP/HTTPS/UDP 公共 Trackers 自动注入，优化代理与 Fake-IP 环境连通性 | ✅ 完成（含单测） | backend |
| BT-03 | 真实速度与状态：每秒快照差值计算瞬时下载/上传速度与精确 ETA，统计真实 peers 与 seeds 数量 | ✅ 完成（含单测） | backend |
| BT-04 | 任务模型与元数据：`task_kind` 字段、磁力 URI 解析、元数据就绪后动态回填文件名与文件树 | ✅ 完成（含单测） | backend |
| BT-05 | 做种控制与分享率：做种默认关闭（完成后自动停止上传），支持自定义做种分享率 | ✅ 完成（含单测） | backend |
| BT-06 | 文件选择下载：详情面板文件树与勾选，对接 `only_files` 局部下载 | ✅ 完成（含单测） | backend |
| BT-07 | 前端与详情：任务行 BT 徽标、详情栏 BT 连接与文件树勾选面板、设置页 BT 分区、i18n zh/en 全覆盖 | ✅ 完成 | frontend |
| BT-08 | 扩展接管：`magnet:` 链接点击/划词接管（send-magnet 契约、离线/未配对安全回退） | ✅ 完成 | frontend |
| BT-09 | 质量门禁与测试：磁力解析、Tracker 注入、元数据过渡、做种停止、ETA 计算等全量单测 100% 绿灯 | ✅ 完成 | qa |
