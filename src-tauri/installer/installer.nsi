...
Function InstallOptionsShow
  ${If} $PassiveMode = 1
    Abort
  ${EndIf}
  !insertmacro MUI_HEADER_TEXT "安装选项" "选择猫步下载器的启动方式"
  nsDialogs::Create 1018
  Pop $0
  ${NSD_CreateCheckbox} 0 10u 100% 12u "开机自动启动 猫步下载器（可随时在 设置 → 通用 中关闭）"
  Pop $AutoStartCheckbox
  ${NSD_Check} $AutoStartCheckbox
  ${If} $AutoStartState == 0
    ${NSD_Uncheck} $AutoStartCheckbox
  ${EndIf}
  nsDialogs::Show
FunctionEnd
