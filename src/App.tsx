...
export default function App() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>({ size: 78, status: 92, progress: 140, speed: 78, eta: 82, created: 86 });
  ...
  const beginResize = (key: string, event: MouseEvent) => {
    event.preventDefault(); event.stopPropagation(); const start = event.clientX; const width = columnWidths[key];
...
function NewTaskDialog({ settings, onClose, onCreated }: { settings: AppSettings; onClose: () => void; onCreated: (count: number) => void }) {
  ...
  return <Modal title="新建下载任务" onClose={onClose} wide><div className="new-task-form">
    ...
    {toolStatus && toolStatus.state !== "ready" && <MediaToolsCard status={toolStatus} compact onStatus={setToolStatus} />}
...
function SettingsPage({ value, onChange, onClose, notify }: { value: AppSettings; onChange: (value: AppSettings) => void; onClose: () => void; notify: (text: string, kind?: "ok" | "error") => void }) {
  ...
  return <div className="settings-page"><aside className="nav-pane"><div className="brand">设置</div>{items.map(([key,label,Icon]) => <button key={key} className={section === key ? "nav-item active" : "nav-item"} onClick={() => setSection(key)}><Icon size={15} /><span>{label}</span></button>)}<button className="nav-settings" onClick={onClose}><X size={15} /><span>返回任务</span></button></aside><main className="settings-body"><div className="settings-title"><h1>{items.find(([key]) => key === section)?.[1]}</h1></div><div className="settings-content">
    ...
    {section === "media" && <SettingsGroup title="媒体工具"><p className="settings-note">普通下载无需额外组件。媒体分析需要按需安装 yt-dlp 与 FFmpeg；DRM 内容不会处理。</p>{tools ? <MediaToolsCard status={tools} onStatus={setTools} /> : <LoaderCircle className="spin" />}<Toggle label="自动检查媒体工具更新" checked={draft.media_tool_auto_update} onChange={(v) => set("media_tool_auto_update", v)} /></SettingsGroup>}
...
function MediaToolsCard({ status, onStatus, compact = false }: { status: ToolStatus; onStatus: (value: ToolStatus) => void; compact?: boolean }) {
  ...
  return <div className={compact ? "media-tools-card compact" : "media-tools-card"}><div className="tool-summary"><span className={`tool-state ${status.state}`}>{status.state === "ready" ? <Check size={14} /> : active ? <LoaderCircle className="spin" size={14} /> : <Video size={14} />}</span><div><strong>{status.state === "ready" ? "媒体工具已就绪" : active ? "正在安装媒体工具" : "安装媒体工具"}</strong><small>{status.version}</small></div><em>{status.state === "ready" ? formatBytes(status.installed_bytes) : "下载约 122 MB · 安装约 216 MB"}</em></div>{active && <div className="tool-progress"><div><i style={{ width: `${progress}%` }} /></div><span>{status.state === "verifying" ? "正在校验" : status.state === "extracting" ? "正在解压" : `${formatBytes(status.downloaded_bytes)} / ${formatBytes(status.total_bytes)}`}</span></div>}{status.error && <p className="tool-error">{status.error}</p>}<div className="tool-actions">{status.state === "ready" ? <><button onClick={() => void api.checkMediaToolsUpdate().then(onStatus)}>检查更新</button><button className="danger" onClick={() => void api.removeMediaTools().then(() => api.toolStatus().then(onStatus))}>卸载</button></> : active ? <button onClick={() => void api.cancelMediaTools()}>取消安装</button> : <button className="primary" onClick={() => void install()}>确认下载并安装</button>}</div></div>;
}
